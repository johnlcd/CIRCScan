...
def comb_anno_pair():
	if '-h' in opt:
		print 'Arguments:\n','-t: Cell type\n','-f: Feature (exclude hi-c)\n','-r: reference introns and intron pairs\n'
		pass
	else:
		...
		with open('pair_anno_' + CELL + '_' + FEA,'w') as a:
			with open('intron_anno_' + CELL + '_' + FEA,'r') as b:
				with open(RIP,'r') as c:	
					...
					n = 0
					for pair in Pair:
						wl = [pair] + ['0.000']*len(Feature)
						i1 = P1.get(pair)
						i2 = P2.get(pair)
						li1 = L.get(i1)
						li2 = L.get(i2)
						for f in Feature:
							score1 = li1[l1.index(f)]
							score2 = li2[l1.index(f)]
							score = '%.3f' % mt.sqrt(float(score1) * float(score2))
							wl[wl1.index(f)] = str(score)
							n+=1
						if n%100000 == 0:
							print '>>> Intron pair number is:\t', n
							print '    Intron pairs are:\t', i1, ',', i2
							print '    Pair name is:\t', pair, '\n'
						a.write('\t'.join(wl) + '\n')
					print '    Annotaion of %d intron pairs finished.\n' % n
