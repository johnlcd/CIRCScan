...
order_srpbm <- order(data_train[,'SRPBM'], decreasing = F) # remove outlier circRNAs (highest and lowest 30)
if (cell=="NHA") {
	rmn <- 10
} else {
	rmn <- 30
...
data_train <- data_train[order_srpbm[-c((1:rmn),((all_num-rmn+1):all_num))],]
if (PM == 'glm') {
